...
//not working. must reread API slidshow
...
//temporary table
//may adjust table to hold information
...
render() { 
    ...
    <React.Fragment>
        ...
        {this.state.songs.map((element) => <><tbody><tr class="this-row"><td>{element.title}</td> <td>{element.artist}</td> <td>{element.album}</td><td>{element.release_date}</td><td>{element.genre}</td><td><button onClick={() => this.deleteSong(element)}>Delete</button></td></tr></tbody></> )}      
            </table>
